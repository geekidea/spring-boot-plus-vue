  ...
  export default {
    ...
    data() {
      return {
        ...
        updateId: null,
        rules: {
          // TODO 如果选择的是按钮，必须选择父菜单
          // TODO 菜单路径必须以/开头
          name: [
            { required: true, message: '请输入菜单名称', trigger: 'blur' },
            { min: 4, max: 10, message: '长度在 4 到 16 个字符', trigger: 'blur' }
          ],
          code: [
    ...
    methods: {
      ...
      addMenu() {
        ...
        const parentIds = this.form.parentIds;
        const parentId = (parentIds && parentIds.length === 1) ? parentIds[0] : parentIds[1]
        alert(parentId)
        const addParam = {
          name: this.form.name,
          code: this.form.code,
